...
private:
    ...
    // Uniform fallback emission for unseen observation buckets
    double unknown_emit() const {
        int O = observations.size();
        return (O > 0) ? (1.0 / O) : 1.0;
    }

public:
    BehaviorHMM(const std::vector<std::string>& s) : states(s) {
        int S = states.size();
        for (int i = 0; i < S; i++)
        ...
        emit.assign(S, std::vector<double>());

        start_count.assign(S, 0.0);
        trans_count.assign(S, std::vector<double>(S, 0.0));
        emit_count.assign(S, std::vector<double>());
    }
    ...
    void rebuild() {
        ...
        double st_sum = 0.0;
        for (double x : start_count) st_sum += x + 1.0;
        for (int i = 0; i < S; i++)
    ...
    std::vector<std::string> decode(const std::vector<RawObservation>& obs) {
        ...
        // Map observations to indices; -1 means unseen bucket
        std::vector<int> oidx(T);
        for (int t = 0; t < T; t++) {
            std::string b = obs[t].to_bucket();
            oidx[t] = obs_idx.count(b) ? obs_idx[b] : -1;
        }
        ...
        // Initialisation
        for (int i = 0; i < S; i++) {
            double e = (oidx[0] >= 0) ? emit[i][oidx[0]] : unknown_emit();
            dp[0][i] = log_safe(start[i] + 1e-12) + log_safe(e + 1e-12);
        }
        ...
        for (int t = 1; t < T; t++) {
            for (int j = 0; j < S; j++) {
                ...
                double e = (oidx[t] >= 0) ? emit[j][oidx[t]] : unknown_emit();
                dp[t][j]   = best + log_safe(e + 1e-12);
                back[t][j] = best_i;
            }
        }
    ...
    std::vector<std::pair<std::string, double>> predict_next_observation(
